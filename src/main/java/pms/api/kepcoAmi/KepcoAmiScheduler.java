...
public class KepcoAmiScheduler {
    ...
    public void getKepcoAmiData() throws URISyntaxException {
        ...
        //계기 번호 응답용 List
        List<KepcoAmi> amiList = new ArrayList<>();
        //고객 번호 응답용 List
        List<KepcoAmi> customerList = new ArrayList<>();
        //데이터 파싱용 Type 설정
        ...
        try {
            ...
            logger.error("한전 계기 번호 데이터 수신 실패.");
            logger.error(e.getLocalizedMessage());
        } catch (ResourceAccessException e) {
            logger.error("한전 계기 번호 데이터 호출시 TimeOut 발생");
            logger.error(e.getLocalizedMessage());
        } catch (JsonSyntaxException | ClassCastException e) {
            logger.error("meterApiResponseApiBody - 한전 계기 번호 데이터 JsonSyntaxException | ClassCastException");
            logger.error(e.getLocalizedMessage());
        } catch (Exception e){
            logger.error("한전 계기 번호 데이터 Exception 발생.");
            logger.error(e.getLocalizedMessage());
        }
        ...
        try {
            ...
            logger.error("한전 고객 번호 데이터 수신 실패.");
            logger.error(e.getLocalizedMessage());
        } catch (ResourceAccessException e) {
            logger.error("한전 고객 번호 데이터 호출시 TimeOut 발생");
            logger.error(e.getLocalizedMessage());
        } catch (JsonSyntaxException | ClassCastException e) {
            logger.error("customerApiResponseBody - 한전 고객 번호 데이터 JsonSyntaxException | ClassCastException");
            logger.error(e.getLocalizedMessage());
        } catch (Exception e){
            logger.error("한전 고객 번호 데이터 Exception 발생.");
            logger.error(e.getLocalizedMessage());
        }
        ...
        if(amiList.size() > 0 && customerList.size() > 0) {
            ...
            for (KepcoAmi ami : amiList) {//url 정의 (서비스 키, 데이터 타입, 고객번호 SET
                ...
                try {
                    ...
                    logger.error("고객 번호 : " + ami.getCustNo() + " , 계기 번호 : " + ami.getMeterNo());
                    logger.error(minuteUseApiResponse.toString());
